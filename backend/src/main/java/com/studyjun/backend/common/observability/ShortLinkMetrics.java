package com.studyjun.backend.common.observability;
...
public class ShortLinkMetrics implements RedirectMetrics {
    ...
    @Override
    public void incrementRedirectCacheHit() {
        redirectLookupCacheHitCounter.increment();
    }

    @Override
    public void incrementRedirectCacheMiss() {
        redirectLookupCacheMissCounter.increment();
    }

    @Override
    public void incrementNegativeCacheHit() {
        redirectLookupNegativeCacheHitCounter.increment();
    }

    @Override
    public void incrementNegativeCacheMiss() {
        redirectLookupNegativeCacheMissCounter.increment();
    }

    @Override
    public void incrementRedirectDbFallback() {
        redirectLookupDbFallbackCounter.increment();
    }

    public Timer redirectLatencyTimer() {
        return redirectLatencyTimer;
    }

    public void incrementKafkaPublishSuccess() {
    ...
    public void setKafkaConsumerLag(long lag) {
        kafkaConsumerLag.set(Math.max(lag, 0));
    }

    public void setRedisCounterKeyCount(int count) {
        redisCounterKeyCount.set(Math.max(count, 0));
    }

    public void incrementFlushExecution() {
        flushExecutionCounter.increment();
    }

    public void incrementFlushSuccess() {
        flushSuccessCounter.increment();
    }

    public void incrementFlushFailure() {
