...
const CODE_START = Number(__ENV.CODE_START || 1);
const CODE_COUNT = Number(__ENV.CODE_COUNT || 100);

const DISABLE_THRESHOLDS = (__ENV.DISABLE_THRESHOLDS || 'false').toLowerCase() === 'true';
const LOCATION_MATCH_MODE = (__ENV.LOCATION_MATCH_MODE || 'off').toLowerCase();
...
function randomCode() {
  const id = Math.floor(Math.random() * CODE_COUNT) + CODE_START;
...
export default function () {
  ...
  const response = http.get(`${BASE_URL}/s/${code}`, {
    redirects: 0,
    tags: {
      endpoint: 'redirect-random-full-path',
      scenario: 'random',
      mode: 'full-path',
